...
```bash
head -n1 braker_utr.aa.out.outfmt6.w_pct_hit_length | sed 's/^#/Annotation\t/' > $SPECIES.AbInitioVsRef.w_pct_hit_length.tsv
```

Now you can append all the `tsv` files to the final `tsv` file:
```bash
...
```bash
Analyze_Diamond_topHit_coverage.R $SPECIES.AbInitioVsRef.w_pct_hit_length.tsv $SPECIES.AbInitioVsRef.w_pct_hit_length.png
```
